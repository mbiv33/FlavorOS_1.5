...
description: >-
  Scooter — Ripple-observation pass. Run at the end of any artifact-producing
...
> **About this skill:** Ripple-observation is a *protocol every specialist runs*. Each consuming agent (Scooter, Sinclair, Maxine, Kyle) gets its own scoped copy. They share the observation queue (`vault/00-Inbox/ripple-observations/`) and the ripple SIGMA type. Khadijah holds the synthesis pass.

## When to run

Always run as the **final step** of any artifact-producing skill in Scooter's domain:
...
- **travel-planning** / **travel-itinerary** (Scooter) — primary callers
- **ripple-observation** (Sinclair, Maxine, Kyle — future) — same protocol, scoped to each agent's domain
