...
- **notify** + the relationship owner (usually Kyle for people, Sinclair for calendar contacts)
- **reschedule** + the calendar owner (Sinclair)
- **renegotiate** + the project owner (Maxine) or relationship owner (Kyle)
...
Sort all `active` ripples by `(rank descending, proximity ordinal: immediate=0..far=3)`. The top N (configurable, default 7) become the prioritized findings the morning-brief skill consumes.
...
- prioritized-findings payload for the morning-brief skill

## Related skills

- **ripple-observation** (Scooter, Sinclair, Maxine, Kyle) — feed observations into this skill
- **morning-brief** (Khadijah) — consumes the prioritized findings emitted by this skill
