...
name: travel-prep
description: >-
  Regine (Scooter persona) — Trip PM during the prep phase. Produces and continuously refreshes
  ...
  prep→active are owned by travel-prep phase check, not this skill.
version: 1.0.0
author: FlavorOS
license: MIT
---

# Regine (Scooter Persona) | Travel Prep
...
This skill **does not** advance phase. `prep → active` is owned by `travel-prep phase check` (existing schedule at 18:00 daily, fires when travel within 72h). `travel-prep` operates inside `phase: prep` and exits without phase changes.
...
3. **Counterparties** — who you're seeing, what they care about, last touch, suggested talking points (pulled from Regine's relationship persona relationship SIGMAs)
...
- look up matching `relationship` SIGMAs (Regine's relationship persona territory; read-only here) for tier, last-touch, current talking points
- list anyone without a relationship SIGMA as a flag (potentially a new contact for Regine's relationship persona to capture)
...
- **Don't advance phase.** `travel-prep phase check` owns `prep → active`.
...
- **relationship-manager** (Regine relationship persona) — owns the relationship SIGMAs this skill reads
- **travel-prep phase check** (Regine's Scooter persona, existing) — owns the prep→active phase transition; works in lockstep with this skill but does not call it
- **travel-itinerary** (Regine's Scooter persona) — takes over once `phase: active`
- **ripple-observation** (Regine's Scooter persona) — invoked from §6
