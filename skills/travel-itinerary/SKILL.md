...
description: >-
  Regine (Scooter persona) — Live trip tick. While a trip is active, mutate its SIGMA with
...
# Regine (Scooter Persona) | Live Itinerary
...
Divergences are append-only. Don't mutate `entities.bookings` to absorb a divergence — record both: the booking remains the *expected* state, the divergence captures *actual*. The user (or `travel-itinerary`) decides whether to update the booking, accept the new state, or push back on the vendor.
...
Khadijah picks the channel and timing for any user-facing message. Regine's Scooter persona never DMs client.
...
- **Never auto-cancel or auto-rebook anything.** Even with high-confidence evidence (e.g., airline emails "your flight is canceled"), the skill records the divergence and flags Khadijah. Rebooking is `travel-itinerary` (future) and always passes through approval.
- **Never advance phase.** `active → return` and `return → debrief` are owned by separate skills (`travel-prep phase check` handles `prep → active`; debrief skills handle later transitions).
...
- **travel-receipts** (Regine's Scooter persona) — coordinates on Tier 3 receipt capture (idempotent — both can record; SIGMA dedupes)
- **travel-booking** (Regine's Scooter persona, future) — populates `entities.bookings` that this skill watches
- **travel-itinerary** (Regine's Scooter persona, future) — handles modifications when divergences require rebooking
- **travel-debrief** (Regine's Scooter persona, future) — reads observations + divergences at trip close
