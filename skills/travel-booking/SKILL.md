...
name: travel-booking
description: >-
  Regine (Scooter persona) — Booking execution. Activates after Khadijah marks the Plan
...
# Regine (Scooter Persona) | Travel Booking
...
- **Never re-book without explicit instruction.** If a confirmed booking needs to change, that's `travel-itinerary` (future skill), not this one.
- **Never advance phase past `prep`.** Even with all bookings confirmed, this skill stops at `prep`. The active phase begins via `travel-prep phase check` on the depart day.
...
- **travel-planning** (Regine's Scooter persona) — produces the Plan this skill consumes
- **travel-itinerary** (Regine's Scooter persona) — once `phase: active`, watches for booking divergences (delays, cancellations)
- **travel-itinerary** (Regine's Scooter persona, future) — handles modifications after `entities.bookings` exists
- **ripple-observation** (Regine's Scooter persona) — invoked from §7
