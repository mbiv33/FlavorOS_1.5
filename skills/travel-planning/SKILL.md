...
description: >-
  Regine (Scooter persona) — Travel Planning. Convert a trip request into a researched,
...
# Regine (Scooter Persona) | Travel Planning
...
- consult Khadijah's Maxine persona finance pulse for current budget headroom
...
- **people**: named attendees, contacts, VIPs (Regine's relationship persona `relationship` SIGMAs)
- **places**: destination + origin during the trip window vs. other active commitments
- **work**: project deadlines, milestones (Khadijah's Maxine persona `project-state` SIGMAs)
...
- vault: `05-SIGMA/travel-preferences/`, `05-SIGMA/destination-intelligence/`, `05-SIGMA/vendor-intelligence/`, `05-SIGMA/relationship/` (read-only via Regine's relationship persona domain), `05-SIGMA/project-state/` (read-only via Khadijah's Maxine persona domain), `50-Travel/<slug>/`
...
- **financial-management** (Khadijah / Maxine persona) — budget headroom check
- **relationship-manager** (Regine relationship persona) — VIP ripple check
- **web-research** (Regine's Scooter persona) — invoked from inside this skill
- **logistics-research** (Regine's Scooter persona) — invoked from inside this skill
- **travel-booking** (Regine's Scooter persona, future) — runs only after Plan is approved
