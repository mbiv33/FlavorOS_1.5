...
name: travel-return
description: >-
  Regine (Scooter persona) — Return protocol. Owns the active→return phase transition. On
...
# Regine (Scooter Persona) | Travel Return
...
- **Do not resolve open items here.** This skill compiles, doesn't act. Resolution is owner-agent work (Khadijah's Maxine persona for project candidates, Sinclair for reminders, Regine's relationship persona for relationship follow-ups).
...
- **travel-debrief** (Regine's Scooter persona) — fires T+1, picks up where this leaves off
- **ripple-observation** (Regine's Scooter persona) — invoked from §5
- **daily-task-prep** (Khadijah / Maxine persona) — picks up the new-project candidates flagged here
