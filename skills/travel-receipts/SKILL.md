...
description: >-
  Regine (Scooter persona) — Receipt capture and reminders. Multi-tier triggers (cron baseline,
  ...
  Escalates non-response to morning briefing and finally to a Khadijah task.
version: 1.0.0
author: FlavorOS
license: MIT
---

# Regine (Scooter Persona) | Travel Receipts
...
- Khadijah delivers the prompt via the channel she's configured (text, voice, group bot — Regine's Scooter persona does not pick the channel).
- Track the request:
...
- Surface in the next morning briefing (publish on `report.scooter` with severity hint for Khadijah's morning-standup-briefing skill).
- After `prompted_count >= 3` and still missing → emit `flag.medium` and create a Khadijah task: "reconcile expenses — receipts missing on `<trip_id or date>`". This closes Regine's Scooter persona loop; reconciliation is then in finance ops territory.
...
- **Never categorize receipts for tax purposes.** That's Khadijah's Maxine persona territory once she's involved.
...
- **Don't escalate past 3 prompts.** After that, hand to Khadijah's Maxine persona and stop nagging.
...
- on persistent miss: `flag.medium` + Khadijah task
...
- **financial-management** (Khadijah / Maxine persona) — receives the escalation when reconciliation is needed
